...
For each new forecast that you choose to generate, you will first need to create a new folder to hold the output files generated by Amazon Forecast.   You
...
**Step 2: Building the Forecast Infrastructure**
  ...
  b) Navigate to the Cloudformation console.  Select "Create Stack, with new resources (Standard)".  For the template source, choose 'Upload a template file' .   Select the template you downloaded in the previous step. 
...
During Module One of this workshop, you cleaned and enhanced the raw meter data.   Now you need to shape the raw data into the standardized format required Amazon Forecast.   That shaping is performed by executing SQL queries in Amazon Athena.  Those queries are stored in system parameters so that the forecast pipeline can executed them automatically, at the appropriate step in the process.
