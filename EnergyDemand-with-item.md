...
10. Use Athena to shape the raw data into a format that matehes the shape of the TTS file. Note: before you run this, make sure you select the right database from the console drop-down. Also override the athena query output temporarily to go to the raw-data folder. Use the following query as an example:

	 ```
     create table london_meter_data_with_block as SELECT regexp_extract("$path", '[ \w-]+?(?=\.)') as "block_id", lclid as "item_id", energy_sum as "target_value", date_add('year', 6, DATE(day)) as "timestamp" FROM "AwsDataCatalog"."raw-data"."london_meter_data"
     ```
    This query generates a new version of the raw London meter data which conforms to the TTS item schema and . 
...
11. In the raw data folder, create a new sub-folder ("london_meter_info_cleaned") to hold the grid metadata.  Upload the meter-to-grid-info csv file to this new folder.  This is a synthetic dataset which can be used to apply a lat_long value to each TTS record and also to generate an item file.    Run a crawler to create an Athena table so this file can be queried.  Check the schema afterwards to make sure it has field names for each field (see first row of csv file for column names).   

12. Copy the SQL statement below and paste it into the TTS query parameter in Parameter Store.  

	 ```
      SELECT a.item_id, a.target_value, a.timestamp, b.lat_long, a.block_id FROM "AwsDataCatalog"."raw-data"."london_meter_data_with_block" as a  left join "AwsDataCatalog"."raw-data"."london_meter_info_cleaned" as b on a.block_id=b.block_id;
     ```
